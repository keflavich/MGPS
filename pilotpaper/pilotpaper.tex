...
\citep{Wilson2009a,Condon2007a,Condon2016}. 
...
MUSTANG-2 \citep{M2_2014} is a 223 element bolometer array operating on the 100~m
Green Bank Telescope (GBT) with a wide (75--105~GHz) bandwidth and a 4.25'
field-of-view (fov).  The detectors are TES detectors read out using a
microwave multiplexing readout (umux). Typical observing modes consist of
different on-the-fly mapping scans - either small daisy scans for arcminute
sized targets or larger raster scans in perpendicular directions used in the
data presented in this paper.  Both scan patterns are designed to maximize
cross-linking on many timescales so as to enable the removal of 1/f noise from
the instrument and the atmosphere.  In the large bandwidth of MUSTANG-2, line
contamination is generally negligible.
...
the Green Bank Telescope with 9 \arcsec resolution.   When complete, this survey
...
\begin{enumerate}
    ...
    \item A map is made (in azimuth/elevation coordinates) of each calibration
        point source scan in IDL.  Romero et al (in prep) will present the IDL pipeline for \MUSTANG (MUSTANG IDL Data Analysis System, MIDAS)
        \begin{enumerate}
            ...
            \item The beam volume is calculated from the map within a 60"
                aperture as well as analytically from the double 1D Gaussians. The
...
Maps were made using MUSTANG2's MINKASI\citep{Seivers et.al. in Prep} data reduction pipeline which is based on maximum likelihood.  We used smoothed power spectra from a singular value
...
By subdividing timestreams and taking power spectra of each segment, it is possible to identify parts of the power spectra with strong sources as there is a significant increase in the signal band ($\sim$0.1--15~Hz). These regions are flagged and an average power spectrum calculated from the median of the remaining segments. 
...
\propto \nu^{-0.12}$ \citep{Wilson2009a}.  If the power law has a more strongly negative spectral index, when the scaled 20\,cm data is subtracted from the 3\,mm MGPS data, the resultant ``3\,mm Dust'' image will have negative values; if it is more positive, the resultant image will have positive values.  The observed fields were selected
...
emission in these HII regions \todo{Is there any other explanation?  That seems
like kind of a big deal.}(CITE: Padovani+ accepted on synchrotron emission from
HII regions; \url{https://synchrotron-hiiregions.herokuapp.com/}).  However,
...
\begin{figure*}[htp]
\includegraphics[width=17cm]{pilotpaper/figures/G01_overview_withcatalog.pdf}
...
\begin{figure*}[htp]
\includegraphics[width=17cm]{pilotpaper/figures/G12_overview_withcatalog.pdf}
\caption{\MUSTANG image of the G12 field, including the W33 star-forming region.}
\label{fig:g12overview}
\end{figure*}

\begin{figure*}[htp]
\includegraphics[width=17cm]{pilotpaper/figures/G29_overview_withcatalog.pdf}
...
\begin{figure*}[htp]
    \includegraphics[width=17cm]{pilotpaper/figures/G31_overview_withcatalog.pdf}
...
\begin{figure*}[htp]
\includegraphics[width=17cm]{pilotpaper/figures/G34_overview_withcatalog.pdf}
...
\begin{figure*}[htp]
\includegraphics[width=17cm]{pilotpaper/figures/G43_overview_withcatalog.pdf}
...
The W49B supernova remnant in the G43 field stands out as a bright nonthermal source.  No other SNRs in the surveyed area as as bright at 3\,mm (see Figure~\ref{fig:g43overview}). \citet{Sun2011} found that the spectral energy distribution of W49B is well-fit by a single power law from 200\,MHz to 30\,GHz of index $-0.46\pm0.01$.  They found that the 5\,GHz integrated flux density is $19.10\pm0.98$\,Jy, and the 90\,GHz integrated flux density should be 5.1\,Jy. %(90/5)^(-0.46)*19.1 . 
%We therefore expect
Integrating over W9B, we find a flux density of 5.2\,Jy, indicating that all the associated emission is nonthermal. 
