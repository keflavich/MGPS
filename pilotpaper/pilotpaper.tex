...
\begin{document}
\title{The \MUSTANG Galactic Plane Survey (\MGPS) pilot}
\input{authors}

\begin{abstract}
We report the results of a pilot program for a Green Bank Telescope (GBT)
\MUSTANG Galactic Plane survey at 3 mm (90 GHz), \MGPS.  The survey 
...
cataloging, and comparison with other data sets.  We have identified \nsources\
 sources over seven observed fields selecting some of the most
...
\edit{1-3}\,THz, \edit{placing the 90 GHz \MGPS observations firmly on the
...
we expect to see dust emission at 90\,GHz in the \MGPS fields.  
...
(fov).\footnote{\url{http://www.gb.nrao.edu/mustang/}}  The Transition Edge
Sensor (TES) detectors are 
...
The images from this project are released at \dataset[10.7910/DVN/HPATJB]{https://doi.org/10.7910/DVN/HPATJB}
...
In \edit{G34}, only 6 of the constant-latitude scans were completed, so only the bottom
...
\begin{enumerate}
    ...
    \item A map is made in IDL (in azimuth/elevation coordinates) of each
        ...
        (pointlike) source}.  \cite{Romero2020a} describe in detail the
        ...
        \begin{enumerate}
            ...
            \item \edit{The beam solid angle is calculated both from the fitted
                model parameters and from the sum \edit{of pixel values} within
                a 60\arcsec aperture.  These measurements were consistent, so
                we used the analytically
...
Maps \edit{of the science fields} were made using \MUSTANG's MINKASI (J. Sievers
et al. in prep\footnote{\url{https://github.com/sievers/minkasi}}) data
reduction pipeline which is based on the maximum
...
these images showed the closest morphological match to the \MGPS data.
...
the \MUSTANG data.  \edit{The mean and standard deviation offset from the 20 cm
data are $\Delta\ell=1.8\pm2\arcsec$ and $\Delta b=4.4\pm2.7\arcsec$,
respectively.}
...
radius, it was masked out with a single-beam-FWHM circle.  
% We classified
% sources as `compact' if they have major axes $\sigma_{maj}<14\arcsec$, `extended' if
% they have major axis $\sigma_{maj}>14\arcsec$, and `filamentary' if they have
% $\sigma_{maj}/\sigma_{min}>1.5$ and $\sigma_{maj} > 14\arcsec$.

A total of \nsources sources were identified across the seven fields.
\edit{Of these, the majority, \nextended were extended and round ($\sigma_{maj} >
14\arcsec$), and an additional \nfilamentary had both long aspect ratios
$\sigma_{maj}/\sigma_{min} > 1.5$ and were extended ($\sigma_{maj} >
14\arcsec$)}.
Only \ncompact sources were compact \edit{($\sigma_{maj} < 14\arcsec$)}.  Note that any confused or
...
CSO-Bolocam BGPS     & 1100           &  33                  & 50  & \citet{Rosolowsky2010a}\\
                                                             & &&& \citet{Ginsburg2013a} \\
GBT-\MUSTANG \MGPS  & 3274           &  9                   & 1-2  & This work \\
\hline
                     & cm             &                      &   & \\
\hline
...
Of the \nsources total \MGPS sources, \change{\noksources passed our selection criteria that
the peak signal-to-noise ratio in the source was $SNR>5$ and the peak signal was at least
twice that of the background, $S_{peak} > 2 S_{background}$.}.
Of those, \mmdetections had millimeter/submillimeter
...
wavelengths but not at longer wavelengths, \edit{and \cmdetectionsmmnondetections
with long-wavelength matches but no short-wavelength}.
...
\begin{figure*}[htp]
    ...
    while orange (overlaid \edit{as a foreground layer}) shows only those sources that have
...
\begin{enumerate}
    ...
    \item
        ...
        \begin{equation}
            ...
            {1.11~\mathrm{mm}}\right)^{-\alpha} = 0.039
        \end{equation}\\
...
While we would expect some \edit{free-free} excess \edit{at 90 GHz} above the
dust emission \edit{extrapolated from 1.1 mm in} dusty \hchii regions, it is
plausible that the excess is not enough to modify the spectral index to meet
our selection criterion 2.  Sources that have
...
criteria \citep{Sewilo2004a,Avalos2006a}.  \MGPS is clearly capable of detecting
...
within the \MGPS beam \citep{Avalos2009a,Sewilo2011a}.  The source passed the
...
\label{sec:freefree}
As stated in the introduction, the \MGPS data have contributions from thermal
free-free, thermal dust continuum, and nonthermal synchrotron emission.  We
describe here our decomposition of the \MGPS data into free-free and dust
...
ATLASGAL 870\,$\mu$m and \MGPS data flux densities are related via $S_{\rm
...
\MGPS map to obtain an estimated free-free map.  We perform this subtraction
on the feathered \MGPS and Planck data (Section~\ref{sec:feather}).

Similarly, we use 20\,cm maps to estimate the dust contribution by subtracting
a scaled 20\,cm map from the \MGPS data.
...
Figure~\ref{fig:arches_freefree}; the rest of the \MGPS fields are in the
Appendix.  Figure~\ref{fig:arches_freefree} contains panels of the \MGPS data,
the contribution to the \MGPS data from thermal dust estimated from ATLASGAL
subtraction, the contribution to the \MGPS data from free-free and synchrotron
emission, 20\,cm data, and
the contribution to the \MGPS data from thermal dust estimated from 20\,cm
subtraction.
...
diffuse emission in the rightmost panel (\MGPS - VLA) is most likely caused by
...
in the \MGPS image.  The \MGPS - 20 cm image is negative in the 20
...
The decomposed images are shown in Figures \ref{fig:arches_freefree},
\ref{fig:w43freefree}, and Appendix \ref{sec:freefreeappendix}.
...
integrated diffuse emission detected in \MGPS is dominated by emission from hot gas rather
...
\begin{figure*}[htp]
    ...
    \caption{Decomposition of the \MGPS data in the G01 field centered on the
    Arches region. 
    All images are displayed on the same intensity scale.  In the G01 field,
    the 20\,cm data have 30\arcsec resolution, so the \MGPS data have been
    smoothed to match
    the resolution of the other images.
    \edit{From left to right,}
    (a; 870 \um scaled) ATLASGAL 870 \um scaled to 3 mm,
    (b; 3 mm free-free) smoothed \MGPS - scaled ATLASGAL,
    (c; 3 mm) \MGPS,
    (d; 3 mm Dust) smoothed \MGPS - scaled VLA 20 cm continuum,
...
\begin{figure*}[htp]
    ...
    \caption{Decomposition of the \MGPS data in the G31 field centered on W43.
    ...
    smoothed to match the \MGPS data to create panels (d) and (e), while the
    \MGPS data are smoothed to match the ATLASGAL data to create panel (b).
}
...
Survey, \MGPS.  When complete, this survey
...
These are Figures \ref{fig:sgrb2freefree}-\ref{fig:w51mainfreefree}.

\begin{figure*}[htp]
    \includegraphics[width=17cm]{figures/G01_sgrb2_5panel.pdf}
    \caption{Decomposition of the \MGPS data in the G01 field centered on Sgr B2.
...
\begin{figure*}[htp]
    ...
    \caption{Decomposition of the \MGPS data in the G12 field centered on W33.
    See Figure \ref{fig:arches_freefree} for a description of the panels.
...
\begin{figure*}[htp]
    ...
    \caption{Decomposition of the \MGPS data in the G29 field.
    See Figure \ref{fig:arches_freefree} for a description of the panels.
...
\begin{figure*}[htp]
    ...
    \caption{Decomposition of the \MGPS data in the G34 field centered on G34.26+0.15.
    See Figure \ref{fig:arches_freefree} for a description of the panels.
...
\begin{figure*}[htp]
    ...
    \caption{Decomposition of the \MGPS data in the G43 field centered on W49A.
    See Figure \ref{fig:arches_freefree} for a description of the panels.
...
\begin{figure*}[htp]
    ...
    \caption{Decomposition of the \MGPS data in the G43 field centered on W49B.
    See Figure \ref{fig:arches_freefree} for a description of the panels.
...
\begin{figure*}[htp]
    ...
    \caption{Decomposition of the \MGPS data in the G49 field centered on W51 Main.
    See Figure \ref{fig:arches_freefree} for a description of the panels.
...
the brightest 20 sources.  \edit{The full catalog will be published
electronically with the paper.}
