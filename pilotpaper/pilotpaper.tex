...
The survey so far achieves a typical $1\sigma$ depth of $1-2$ mJy/beam in a
...
but did not find any confirmed new ones with multiwavelength data.  We identify \mmdetectionscmnondetections sources
...
strong candidates for new \hchii regions. 
...
($\gtrsim80\%$) of the 3 mm emission comes from hot gas, either through
...
Maps were made using \MUSTANG's MINKASI (Seivers et.al. in prep) data reduction
pipeline which is based on the maximum likelihood pipeline written for ACT\cite{D_nner_2012}.
%maximum likelihood is a mathematical defination - see pg 13 of the reference.  It finds the map that, given your knowledge of the noise properties of the data, is most likely to be the true map of the sky.
...
\hchii regions can be distinguished from from older
...
ultracompact or hypercompact HII region clusters \citep[three are parts of W49A][]{De-Pree1997a},
...
\subsection{Lifetime estimation}
...
the same area, our sample includes \mmdetectionscmnondetectionscompact compact \MUSTANG sources with no
centimeter detections, which are our additional candidates from \S
\ref{sec:hiireg}, and four previously-known \hchii regions.  If we include the
W51 \hchii regions, the sample rises to 22.  W49A may also contain more \hchii regions than the three
we include, but their classifications remain uncertain
\citep{De-Pree1997a,De-Pree2004a}.  The inferred lifetime of \hchii
regions, using a sample size of 12-22 \hchii's in the \MGPS fields, is
therefore $16-30\%$ that of \uchii regions, or $6\ee{3} \mathrm{yr} <
t_{\hchii} < 1\ee{5} \mathrm{yr}$.  A more complete assessment from the larger
survey may more tightly constrain these values.
...
regions (ten in W51 alone, possibly several in W49) suggests that \hchii regions
