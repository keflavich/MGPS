...
compact \hii regions.  To reach an optical depth $\tau_{90 GHz} \sim 1$ at 90 GHz, an \hii
...
\begin{enumerate}
    \item A calibration for the detector array, i.e., relative calibration between
        ...
        antenna temperature.
    \item A map is made in IDL (in azimuth/elevation coordinates) of each \change{scan on a calibrator, which is chosen to be an unresolved (point-like) source}.  \cite{Romero2019a} describe in detail the
        IDL pipeline for \MUSTANG (MUSTANG IDL Data Analysis System, MIDAS)
        \begin{enumerate}
            \item A single 2-D Gaussian is fit to the \change{unresolved} source in order to estimate its location. 
            \item Fixing the centroid as found above, a double Gaussian is fit. The two Gaussian components share a common center; the central Gaussian represents the telescope main beam, and the second Gaussian represents the first sidelobe of the beam response. The main beam is allowed to be elliptical in the fit.
    ...
    \item
        \begin{enumerate}
            ...
            \item Conversion to Rayleigh-Jeans brightness temperature (in K; see e.g. \citealt{Condon2016a}) accounts for the beam solid angle. As such,
    ...
    \item Calibration to Jy, conversion from Jy \perbeam to Kelvin, opacities,
        ...
        applied to the processing of the \change{time ordered data taken on the science target (in this case, scans of the galactic plane)}.
\end{enumerate}
...
some is from our knowledge \change {of the optical depth $\tau_{90 GHz}$} during the observations (for which we use archival weather data and models of the atmosphere). 
...
flagged and an average power spectrum is calculated from the median of the
remaining segments. 
...
of 90.19\,GHz for \MUSTANG, \change{as appropriate for $\alpha=3$ (see Section \ref{sec:eff_freq})}.  We use a simple \texttt{feather} procedure \citep{Cotton2017a}
...
The Hi-GAL sensitivity and angular resolution values both rise from 70 to 500 \um.
...
is not so large.  In any case, we adopt a loosely estimated \uchii lifetime \change{within the range} $4\ee{4}~\mathrm{yr} < t_{\uchii} < 4\ee{5}~\mathrm{yr}$.
...
dendrogram source flux for MUSTANG.  The selected SEDs are of a planetary
nebula (Fig. \ref{fig:g29pn}), which exhibits emission at all
...
250, 350 \um) appear as upper limits because the Hi-GAL catalogs broke this
