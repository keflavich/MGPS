...
Target   &      Time (hr) &       Sessions   &  Estimated Noise (mJy) & $\ell$ offset (\arcsec) & $b$ offset (\arcsec) \\
\hline
\input{obstable_data}
%W51      &      1.02      &        01        \\
%W49      &      1.02      &        01, 02    \\
...
The largest angular scale recovered by MUSTANG is approximately the array
size, about an arcminute.  The large angular scale structure is therefore
missing.  To recover those missing scales, we combine the MUSTANG data with
...
structures (1-10\arcmin) are likely to be poorly recovered.
...
but not at longer wavelengths.  The overview figures, Figures \ref{fig:g01overview}-\ref{fig:g49overview},
show these sources overlaid on the images, color and shape coded by their
crossmatched detections.
...
across the seven target regions.  Of this sample, all turned out to be known
...
\mmdetectionscmnondetections additional candidates can be further investigated
with future deeper centimeter surveys of the Galactic plane.
