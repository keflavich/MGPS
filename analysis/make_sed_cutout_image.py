...
def make_sed_plot(coordinate, mgpsfile, width=1*u.arcmin, surveys=Magpis.list_surveys(), figure=None):
    ...
    print("Retrieving MAGPIS data for {0}".format(coordinate))
    images = {survey:getimg(coordinate, image_size=width*1.5, survey=survey) for survey in surveys}
    images = {x:y for x,y in images.items() if y is not None}
    images['mgps'] = [mgps_cutout]

    print("Retrieving HiGal data for {0}".format(coordinate))
    ...
    for ii, (survey,img) in enumerate(images.items()):
        ...
        print("Plotting {0}".format(survey))
        ...
        ax = figure.add_subplot(4, 5, ii+1, projection=ww)
        ax.set_title("{0}: {1}".format(survey, wlmap[survey]))

        if hasattr(img[0], 'header'):
            new_img,_ = reproject.reproject_interp((img[0].data, wcs.WCS(img[0].header).celestial), target_header, shape_out=shape)
        else:
            new_img = img[0].data
        ...
        if 'GLON' in ww.wcs.ctype[0]:
            xpix, ypix = ww.wcs_world2pix(coordinate.galactic.l, coordinate.galactic.b, 0)
        else:
            xpix, ypix = ww.wcs_world2pix(coordinate.fk5.ra, coordinate.fk5.dec, 0)
        ax.set_xlim(xpix - (width/pixscale/2), xpix + (width/pixscale/2))
        ax.set_ylim(ypix - (width/pixscale/2), ypix + (width/pixscale/2))
